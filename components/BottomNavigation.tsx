...
export default function BottomNavigation() {
  ...
  // ТЗ: НЕ вызываем useCart на /quiz и /plan*, чтобы предотвратить лишние запросы
  ...
  // ТЗ: Если на /quiz или /plan*, не вызываем useCart вообще (возвращаем null для компонента)
  // Это предотвращает любые запросы к API корзины на странице анкеты и плана
  if (isOnQuizPage || isOnPlanPage) {
    // ИСПРАВЛЕНО: Логируем для диагностики (только в dev режиме)
    if (process.env.NODE_ENV === 'development') {
      console.log('🚫 BottomNavigation: returning null on /quiz or /plan', {
        currentPath,
        pathname,
        isOnQuizPage,
        isOnPlanPage,
    ...
    return null; // КРИТИЧНО: Не рендерим компонент на /quiz и /plan*
  }
