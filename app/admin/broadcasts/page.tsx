    ...
    <div className="p-8">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 mb-8 shadow-lg">
          <h2 className="text-xl font-bold mb-4">Новая рассылка</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-2">Тип кожи (опционально)</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Возрастная группа (опционально)</label>
                <select
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Запланировать на (опционально)</label>
              <input
                type="datetime-local"
      ...
      <div className="space-y-4">
        {broadcasts.length === 0 ? (
          ...
          broadcasts.map((broadcast) => (
            <div key={broadcast.id} className="bg-white rounded-2xl p-6 shadow-lg">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="text-xl font-bold mb-2">{broadcast.title}</h3>
                  <p className="text-gray-600 mb-4 whitespace-pre-wrap">{broadcast.message}</p>
                  
                  <div className="flex gap-4 text-sm text-gray-500">
                    <span>
                      Статус: <span className={`font-bold ${
                        broadcast.status === 'completed' ? 'text-emerald-600' :
                        broadcast.status === 'sending' ? 'text-blue-600' :
                        broadcast.status === 'scheduled' ? 'text-yellow-600' :
                        'text-gray-600'
                      }`}>
