...
export default function QuizPage() {
  ...
  if (!hasQuestionnaireAnywhere && initCompletedRef.current) {
    ...
    clientLogger.warn('⚠️ Questionnaire not loaded but init completed - showing fallback loader', {
      hasQuestionnaire: false, // questionnaire здесь всегда null в этом блоке
      hasQuestionnaireRef: false, // questionnaireRef.current здесь всегда null в этом блоке
      questionnaireRefExists: false, // questionnaireRef.current здесь всегда null в этом блоке
      initCompleted: initCompletedRef.current,
      initInProgress: initInProgressRef.current,
      loading,
      error: error || null,
    ...
    // ИСПРАВЛЕНО: Показываем второй лоадер только если прошло достаточно времени после завершения init()
    // Это предотвращает показ второго лоадера сразу после первого
    // Используем таймаут 5 секунд - если анкета не загрузилась за это время, показываем второй лоадер
    const timeSinceInitCompleted = initCompletedTimeRef.current 
      ? Date.now() - initCompletedTimeRef.current 
      : 0;
    const shouldShowFallbackLoader = timeSinceInitCompleted > 5000; // 5 секунд
    
    // ИСПРАВЛЕНО: Показываем второй лоадер только после таймаута
    if (!shouldShowFallbackLoader) {
      // Продолжаем показывать основной лоадер (он уже показан выше)
      return null;
    }
    ...
    // ИСПРАВЛЕНО: Показываем лоадер только если анкета НЕ завершена И прошло достаточно времени
    return (
      <div
        style={{
          minHeight: '100vh',
