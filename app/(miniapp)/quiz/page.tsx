...
export default function QuizPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        // ИСПРАВЛЕНО: Сбрасываем isSubmitting для нового пользователя
        // Это предотвращает показ планового лоадера, если isSubmitting остался true из предыдущей сессии
        if (isSubmitting) {
          clientLogger.log('🧹 Сбрасываем залипший isSubmitting при входе на /quiz');
  ...
  // Это гарантирует, что редирект происходит сразу, без показа лоадера анкеты
  if (isSubmitting) {
