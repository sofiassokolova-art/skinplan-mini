...
export default function QuizPage() {
  ...
  // ИСПРАВЛЕНО: Очищаем quiz_just_submitted при входе на /quiz, если анкета не отправляется
  ...
  useEffect(() => {
    try {
      ...
      // этот флаг должен быть снят, иначе увидим лоадер плана
      // ВАЖНО: Очищаем только если анкета НЕ отправляется (isSubmitting === false)
  ...
  // ИСПРАВЛЕНО: Если анкета отправляется, сразу редиректим на /plan
  // Не показываем плановый лоадер на /quiz - он должен быть только на /plan
  if (isSubmitting) {
