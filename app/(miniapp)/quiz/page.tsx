...
export default function QuizPage() {
  ...
  const currentQuestion = useMemo(() => {
    ...
    // ИСПРАВЛЕНО: Порядок проверок соответствует тестам - сначала проверяем все условия для isOnInitialInfoScreen
    ...
    // ИСПРАВЛЕНО: Логика соответствует тестам - проверяем shouldBlockByInfoScreen после вычисления isOnInitialInfoScreen
    const shouldBlockByInfoScreen = (isOnInitialInfoScreen || (pendingInfoScreen && !isRetakingQuiz)) && !showResumeScreen;
    
    // КРИТИЧНО: Логируем несоответствия для диагностики (но не влияем на логику блокировки)
    // ИСПРАВЛЕНО: Эта проверка выполняется только для логирования, логика блокировки уже определена выше
    if ((isShowingInitialInfoScreen && !currentInitialInfoScreen) || 
        (currentInfoScreenIndex >= initialInfoScreens.length && isShowingInitialInfoScreen)) {
      if (!pendingInfoScreen && !showResumeScreen && isDev) {
        clientLogger.warn('⚠️ currentQuestion: обнаружено несоответствие состояний (исправлено через isShowingInitialInfoScreenCorrected)', {
          isShowingInitialInfoScreen,
          isShowingInitialInfoScreenCorrected,
          currentInfoScreenIndex,
          initialInfoScreensLength: initialInfoScreens.length,
        ...
        // НЕ блокируем вопросы, продолжаем выполнение (логика уже обработана через isShowingInitialInfoScreenCorrected)
      }
    }
    
    // ИСПРАВЛЕНО: Проверка shouldBlockByInfoScreen выполняется после всех вычислений, соответствует тестам
    ...
    // ИСПРАВЛЕНО: Убрана избыточная проверка - логика уже обработана через shouldBlockByInfoScreen
    // Если shouldBlockByInfoScreen = false, значит вопросы должны показываться
    // Дополнительные проверки не нужны, так как они уже учтены в isOnInitialInfoScreen и shouldBlockByInfoScreen
    ...
    if (isDev && currentInfoScreenIndex >= initialInfoScreens.length && !pendingInfoScreen && !showResumeScreen && allQuestions.length > 0) {
    ...
    // НО: если индекс >= allQuestions.length и есть ответы - это нормальное состояние (анкета завершена)
    const answersCount = Object.keys(answers).length;
    const isQuizCompleted = currentQuestionIndex >= allQuestions.length && answersCount > 0;
    
    // КРИТИЧНО: Логируем состояние перед возвратом вопроса
    if (isDev && allQuestions.length > 0 && currentQuestionIndex >= 0 && currentQuestionIndex < allQuestions.length) {
      clientLogger.log('✅ currentQuestion: возвращаем вопрос', {
        currentQuestionIndex,
        allQuestionsLength: allQuestions.length,
    ...
    if (currentQuestionIndex < 0) {
      clientLogger.warn('⚠️ currentQuestion: null (index < 0) - корректируем на 0', {
        currentQuestionIndex,
        allQuestionsLength: allQuestions.length,
        isShowingInitialInfoScreen,
        currentInfoScreenIndex,
      ...
      // КРИТИЧНО: Если индекс < 0, устанавливаем 0
      setTimeout(() => {
        if (currentQuestionIndex < 0) {
          clientLogger.log('🔄 Корректируем currentQuestionIndex на 0', {
      ...
      // Временно возвращаем первый вопрос
      if (allQuestions.length > 0) {
    ...
    // ИСПРАВЛЕНО: Если индекс >= allQuestions.length, но анкета не завершена - корректируем
    // Если анкета завершена (есть ответы) - возвращаем null (автоотправка обработает это)
    if (currentQuestionIndex >= allQuestions.length) {
      if (!isQuizCompleted) {
        clientLogger.warn('⚠️ currentQuestion: null (index >= length, но нет ответов) - корректируем', {
          currentQuestionIndex,
          allQuestionsLength: allQuestions.length,
          answersCount,
          isShowingInitialInfoScreen,
        ...
        // Корректируем на последний валидный вопрос
        setTimeout(() => {
          const correctedIndex = Math.max(0, allQuestions.length - 1);
          if (correctedIndex !== currentQuestionIndex) {
            clientLogger.log('🔄 Корректируем currentQuestionIndex на последний валидный', {
              oldIndex: currentQuestionIndex,
        ...
        // Временно возвращаем последний вопрос
        if (allQuestions.length > 0) {
          return allQuestions[allQuestions.length - 1];
        }
      }
      // Если анкета завершена - возвращаем null (автоотправка обработает это)
      return null;
    }
    ...
    return null;
  }, [isShowingInitialInfoScreen, isShowingInitialInfoScreenCorrected, pendingInfoScreen, isRetakingQuiz, currentQuestionIndex, allQuestions, allQuestionsRaw.length, questionnaire, loading, hasResumed, showResumeScreen, answers, savedProgress, currentInitialInfoScreen, initialInfoScreens.length, currentInfoScreenIndex]);
  ...
  if (isShowingInitialInfoScreenCorrected && 
  ...
  // КРИТИЧНО: Если isShowingInitialInfoScreenCorrected = true, но currentInitialInfoScreen = null,
