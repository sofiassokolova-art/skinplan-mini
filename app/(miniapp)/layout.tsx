...
function LayoutContent({
  ...
  // КРИТИЧНО: Скрываем навигацию на главной странице, если isNewUser === null (еще проверяется)
  // Это предотвращает вызов useCart() до того, как определится, новый ли пользователь
  const hideNav = isOnQuizPage || 
                  pathname === '/loading' ||
                  pathname.startsWith('/loading/') ||
                  isResumeScreen ||
                  isPlanGenerating ||
                  (pathname === '/' && (isNewUser === true || isNewUser === null)); // Скрываем навигацию для нового пользователя на главной И пока проверяется
