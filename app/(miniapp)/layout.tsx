...
function LayoutContent({
  ...
  // ОПТИМИЗИРОВАНО: Скрываем навигацию на главной странице дольше, чтобы не вызывать /api/cart для новых пользователей
  useEffect(() => {
    // Не проверяем профиль в layout - это делает главная страница
    // Скрываем навигацию на главной странице до завершения проверки профиля (до 3 секунд)
    // Это предотвращает вызов /api/cart для новых пользователей, которые будут редиректнуты на /quiz
    if (pathname === '/') {
      setIsCheckingProfile(true);
      // Увеличиваем задержку, чтобы дать время главной странице проверить профиль и сделать редирект
      // Если пользователь новый, он будет редиректнут на /quiz до того, как навигация появится
      setTimeout(() => setIsCheckingProfile(false), 3000);
    } else {
