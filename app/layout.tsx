    ...
    try { window.dispatchEvent(new Event('telegram-webapp-ready')); } catch (_) {}
      ...
      >
        ...
        {!isAdminRoute && (
          ...
          >
            {/* Чёрный крутящийся кружок на белом — единый entry-лоадер,
                как до анкеты (QuizInitialLoader). Без лаймового скелетона. */}
            <div
              style={{
                width: 40,
                height: 40,
                ...
                animation: 'skinplan-root-spin 0.8s linear infinite',
              }}
            />
            <style
              dangerouslySetInnerHTML={{
                __html: `
                  @keyframes skinplan-root-spin {
