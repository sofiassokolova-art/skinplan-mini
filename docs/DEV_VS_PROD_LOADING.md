...
## Прод всё ещё не грузит — чеклист

1. **Задеплойте последний main** и дождитесь окончания сборки в Vercel.
2. **В Telegram:** очистите данные мини-приложения (чтобы сбросить старый Service Worker и кэш): закройте бота, в настройках Telegram → Данные и память → очистка кэша или перезапуск. Либо откройте мини-приложение в режиме «открыть в браузере» и там очистите данные сайта.
3. **Проверьте /miniapp-ping:** в Telegram откройте ссылку `https://www.proskiniq.ru/miniapp-ping`. Должна открыться страница с текстом «OK». Если открывается — домен и заголовки в порядке, проблема в загрузке основного приложения (чанки). Если не открывается — проблема в сети/прокси/CSP/домене.
4. **Перед доменом нет кэширующего прокси?** Если используется Cloudflare или другой CDN — отключите кэш для HTML (или для всего домена) для теста.
5. **Переменные Production в Vercel:** `NEXT_PUBLIC_MINI_APP_URL=https://www.proskiniq.ru`, `TELEGRAM_BOT_TOKEN`, `DATABASE_URL`, `JWT_SECRET`.
...
2. **Проверить, что грузится именно прод:** открыть в Telegram **https://www.proskiniq.ru/miniapp-ping** — должна открыться страница с текстом «OK» (без React). Если «OK» есть, домен и заголовки в порядке, проблема в загрузке чанков. Если и `/miniapp-ping` не открывается — смотреть блокировки (CSP, frame-ancestors, сеть).
...
4. **Service Worker:** в коде регистрация SW отключена (`SERVICE_WORKER_ENABLED = false` в `components/ServiceWorker.tsx`). После деплоя у пользователей, у которых уже был установлен старый SW, нужно **очистить данные сайта** в Telegram (Настройки → Данные и память → Использование памяти → выйти/очистить для этого бота или переустановить Telegram), иначе старый SW может продолжать отдавать кэш.
...
## Анкета на localhost отличается от прода

Анкета подгружается из БД: локально используется локальная база, в проде — продовая. Поэтому список вопросов и варианты ответов могут различаться.

**Чтобы локально видеть ту же анкету, что и в проде:**

1. Синхронизировать анкету из прода в локальную БД:
   ```bash
   npm run sync:questionnaire-from-prod
   ```
   По умолчанию скрипт запрашивает анкету с `NEXT_PUBLIC_MINI_APP_URL` или с `https://www.proskiniq.ru`. Чтобы указать другой URL:
   ```bash
   SYNC_QUESTIONNAIRE_FROM_URL=https://www.proskiniq.ru npm run sync:questionnaire-from-prod
   ```
2. Убедиться, что локально используется своя `DATABASE_URL` (локальная БД), чтобы не перезаписывать прод.
