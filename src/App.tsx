...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2 py-1 rounded-full text-xs bg-zinc-100 border border-zinc-200">
      {children}
    </span>
  );
}
...
function buildPlan(answers: any) {
  ...
  if (goals.includes("redness") || (answers.q7_safe || []).includes("rosacea"))
    concerns.add("rosacea");
  ...
  const wantsMild =
    sensitivity || prefs.has("pregsafe") || (answers.q7_safe || []).length > 0;

  type Active = { id: string; name: string; when: string; strength: number };
  const activePool: Active[] = [];
  if (concerns.has("dehydration"))
    activePool.push({
      id: "ha",
      name: "Гиалуроновая сыворотка/эссенция",
      when: "AM/PM",
      strength: 0,
    });
  ...
  if (goals.includes("acne") || concerns.has("texture"))
    activePool.push({
      ...
      name: wantsMild
        ? "Азелаиновая кислота 10%"
        : "Салициловая кислота (BHA) 1–2%",
      when: "PM alt",
  ...
  if (concerns.has("aging") && !prefs.has("pregsafe"))
    activePool.push({
      id: "retinoid",
      name: sensitivity
        ? "Ретиналь/ретинол 0.1–0.3%"
        : "Ретиноид (ретиналь/ретинол)",
      when: "PM",
      strength: 4,
    });
  if (concerns.has("rosacea"))
    activePool.push({
      id: "niacin",
      name: "Ниацинамид 4–10%",
      when: "AM/PM",
      strength: 1,
    });
  ...
  const amActive = activePool.find(
    (a) => a.when.includes("AM") && a.id !== "retinoid" && a.id !== "bha"
  );
  ...
  pm.push("Очищение");
  const pmActive =
    activePool.find((a) => a.when.includes("PM") && a.id !== "ha") ||
    activePool.find((a) => a.when.includes("PM alt"));
  if (!compactPM && pmActive) pm.push(pmActive.name);
  pm.push("Крем/бальзам");

  // порядок по бюджету и безопасный пикер (работает с readonly)
  const order =
    budgetPref === "low"
      ? ["low", "mid", "high"]
      : budgetPref === "mid"
      ? ["mid", "low", "high"]
      : ["high", "mid", "low"];
  const pick = (arr: ReadonlyArray<SuggItem>) =>
    [...arr].sort((a, b) => order.indexOf(a.b) - order.indexOf(b.b))[0]?.n;

  const picks = {
    cleanser:
      pick(
        (SUGG.base.cleanser as any)[type] || SUGG.base.cleanser.normal
      ) || "",
    moisturizer:
      pick(
        (SUGG.base.moisturizer as any)[type] || SUGG.base.moisturizer.normal
      ) || "",
  ...
  const activeMap: Record<string, ReadonlyArray<SuggItem>> = {
  ...
  activePool.forEach((a) => {
    ...
    if (list) {
      ...
      if (title) picks.actives.push(title);
    }
  });
  ...
  if (sensitivity)
    notes.push(
      "Начинайте активы через день, патч-тест перед первым применением."
    );
  if ((answers.q7_safe || []).includes("rosacea"))
    notes.push("Избегайте горячей воды, спиртов/отдушек, резких кислот.");
  if (goals.includes("acne"))
    notes.push("BHA 1–2 раза в неделю, не выдавливайте воспаления.");
  ...
  const conflictRules = [
    {
      id: "retinoid+acidsPM",
      test: "tags.retinoid && (tags.aha || tags.bha)",
      msg: "Не сочетайте ретиноид с AHA/BHA в одну ночь. Разведите по дням.",
    },
    {
      id: "bpo+vitcAM",
      test: "tags.bpo && tags.vitc && context.am && !context.spf",
      msg: "BPO с витамином C утром без SPF — риск раздражения/оксидации.",
    },
    {
      id: "multiAcids",
      test: "tags.aha && tags.bha",
      msg: "Две кислоты одновременно — сократите до одной.",
    },
    {
      id: "dupActives",
      test: "tags.dup > 0",
      msg: "Дубли активов одной группы. Оставьте один продукт на роль.",
    },
  ];
...
export default function App() {
  ...
  const go = (d = 1) =>
    setIdx((i) => Math.min(TOTAL_STEPS - 1, Math.max(0, i + d)));
  ...
  const sendToTelegram = () => {
    ...
    if (tg) {
      try {
        ...
        tg.showPopup?.({
          title: "Отправлено",
          message: "План и график на 28 дней отправлены",
          buttons: [{ type: "close" }],
        });
        setSent(true);
      } catch (e: any) {
      ...
      navigator.clipboard.writeText(payload).then(() => setSent(true));
    }
  };
    ...
    <div className="flex items-center justify-between pb-6">
      <div>
        ...
        <p className="text-base text-stone-600">
          Подбор ухода · мягкий минимализм
        </p>
      </div>
    ...
    <div className="min-h-dvh p-5 sm:p-8 bg-gradient-to-b from-rose-50 via-amber-50 to-stone-100 text-stone-900">
      <div className="max-w-3xl mx-auto">
        ...
        {idx === 0 && (
          <Card
            title="Q1 — Цели (1/12)"
            subtitle="Определи цели на 4–8 недель. Выбери до 3 — так точнее настрою план."
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) =>
                arr.length <= 3 ? setA("q1_goals", arr) : void 0
              }
              onDone={() => go(+1)}
            />
          </Card>
        ...
        {idx === 1 && (
          <Card
            ...
            subtitle="Тип кожи через 2–3 часа после умывания. Если сомневаешься — выбери 'Не знаю'."
            help={
              <>
                Жирная — выраженный блеск. Комбинированная — Т-зона блестит,
                щёки норм/сухие. Сухая — стянутость/шелушение. Если колеблешься
                — выбери «Не знаю».
              </>
            }
          >
        ...
        {idx === 2 && (
          <Card
            title="Q3 — Реактивность (3/12)"
            subtitle="Как кожа реагирует на новые средства?"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                { label: "Нет", key: "none" },
        ...
        {idx === 3 && (
          <Card
            title="Q4 — Зоны (4/12)"
            subtitle="Где основная зона фокуса? Можно несколько."
          >
            <ToggleList
              items={[
        ...
        {idx === 4 && (
          <Card
            title="Q5 — База ухода (5/12)"
            subtitle="Что из базовых шагов уже есть в рутине?"
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) => {
                if (arr.includes("none")) setA("q5_base", ["none"]);
                else setA("q5_base", arr.filter((x: string) => x !== "none"));
              }}
              onDone={() => go(+1)}
            />
          </Card>
        ...
        {idx === 5 && (
          <Card
            title="Q6 — Активы (6/12)"
            subtitle="Какие активы уже используешь? Отметь всё применимое."
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) => {
                if (arr.includes("none")) setA("q6_act", ["none"]);
                else setA("q6_act", arr.filter((x: string) => x !== "none"));
              }}
              onDone={() => go(+1)}
            />
          </Card>
        ...
        {idx === 6 && (
          <Card
            title="Q7 — Safety-факторы (7/12)"
            subtitle="Отметь факторы, влияющие на выбор активов."
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) => {
                if (arr.includes("none")) setA("q7_safe", ["none"]);
                else setA("q7_safe", arr.filter((x: string) => x !== "none"));
              }}
              onDone={() => go(+1)}
            />
          </Card>
        ...
        {idx === 7 && (
          <Card
            title="Q8 — Процедуры 6 мес (8/12)"
            subtitle="Недавние процедуры у косметолога за 6 месяцев?"
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) => {
                if (arr.includes("none")) setA("q8_proc", ["none"]);
                else setA("q8_proc", arr.filter((x: string) => x !== "none"));
              }}
              onDone={() => go(+1)}
            />
          </Card>
        ...
        {idx === 8 && (
          <Card
            title="Q9 — Время на уход (9/12)"
            subtitle="Сколько времени комфортно тратить ежедневно?"
          >
        ...
        {idx === 9 && (
          <Card
            title="Q10 — Бюджет (10/12)"
            subtitle="Бюджет на старт и поддержку. Выбери ориентир — подстрою подбор."
          >
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
        ...
        {idx === 10 && (
          <Card
            title="Q11 — Предпочтения к формулам (опц., 11/12)"
            subtitle="Есть требования к формулам и ощущениям?"
          >
            <ToggleList
              items={[
              ...
              onChange={(arr) => {
                if (arr.includes("none")) setA("q11_prefs", ["none"]);
                else setA(
                  "q11_prefs",
        ...
        {idx === 11 && (
          <Card
            title="Итог (12/12)"
            subtitle="Проверь ответы, сгенерируй план и график 28 дней"
          >
            <div className="space-y-3 text-sm">
              <div className="p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                ...
                <ul className="list-disc pl-5">
                  <li>
                    Цели: {(answers.q1_goals || []).join(", ") || "—"}
                  </li>
                  <li>
                    Тип кожи: {answers.q2_skin || "—"} · Реактивность:{" "}
                    {answers.q3_react || "—"}
                  </li>
                  <li>
                    Время: AM {answers.q9_am || "—"} / PM {answers.q9_pm || "—"}
                  </li>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                <div className="md:col-span-2 p-4 rounded-2xl border bg-white border-zinc-200">
                  ...
                  <div className="text-sm text-zinc-600 mb-3">
                    ...
                    <b>
                      {
                        TYPE_RU[
                          plan.detected_type as keyof typeof TYPE_RU
                        ]
                      }
                    </b>{" "}
                    {plan.sensitivity && <span>· чувствительная</span>}
                  </div>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                      <div className="font-semibold mb-2">Утро</div>
                      <ul className="list-disc pl-5 space-y-1 text-sm">
                        {plan.routine.am.map((s: string, i: number) => (
                          <li key={i}>{s}</li>
                        ))}
                      </ul>
                    </div>
                    <div className="p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                      ...
                      <ul className="list-disc pl-5 space-y-1 text-sm">
                        {plan.routine.pm.map((s: string, i: number) => (
                          <li key={i}>{s}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="text-xs text-zinc-600 mb-2">
                      PM-фазы: <b>A</b> — активная ночь, <b>B</b> — барьерная,
                      <b> Rest</b> — отдых. Бот может присылать напоминания и
                      корректировать частоту по фидбеку.
                    </div>
                    <ol className="list-decimal pl-5 space-y-1 text-sm">
                      {plan.schedule28.slice(0, 7).map((d: any) => (
                        <li key={d.day}>
                          День {d.day}: PM — {d.pm.phase}
                        </li>
                      ))}
                    </ol>
                  </div>
                ...
                <div className="p-4 rounded-2xl border bg-white border-zinc-200">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li>
                      <span className="font-medium">Очищение:</span>{" "}
                      {plan.picks.cleanser}
                    </li>
                    <li>
                      <span className="font-medium">Крем:</span>{" "}
                      {plan.picks.moisturizer}
                    </li>
                    <li>
                      <span className="font-medium">SPF:</span> {plan.picks.spf}
                    </li>
                    <li>
                      <span className="font-medium">Барьер на ночь:</span>{" "}
                      {plan.picks.barrier}
                    </li>
                  </ul>
                  {plan.picks.actives?.length > 0 && (
                    <div className="mt-3">
                      ...
                      <ul className="list-disc pl-5 space-y-1 text-sm">
                        {plan.picks.actives.map((a: string, i: number) => (
                          <li key={i}>{a}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="pt-4 flex flex-col gap-2">
                    ...
                    {sent && (
                      <div className="text-xs text-green-600">
                        Готово! Данные отправлены/скопированы.
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="flex justify-between mt-4">
          <button
            ...
            className={cx(
              ...
              idx === 0
                ? "border-white/70 bg-white/50 text-stone-400"
                : "border-white/70 bg-white/60 backdrop-blur shadow-sm"
            )}
          >
            Назад
          </button>
