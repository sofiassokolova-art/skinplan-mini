...
const load = (): CartItem[] => {
  try { 
    ...
    console.log('Загружаем корзину из localStorage:', raw);
      ...
      <section className="bg-white/70 border border-white/60 rounded-3xl p-6 backdrop-blur-xl">
        ...
        {items.length===0 ? (
          <div className="text-zinc-600">
            Корзина пуста. Перейди в <button onClick={() => navigate("/plan")} className="underline text-indigo-600 hover:text-indigo-800 transition">План</button> и добавь всё в корзину.
          </div>
        ) : (
