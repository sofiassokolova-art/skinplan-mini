  ...
  const questionsData = [
    ...
    // 11-14. Особенности кожи
    {
      ...
      questions: [
        ...
        },
        {
          ...
          position: 2,
          isRequired: true,
          // Сокращено с 12 до 8 опций:
          // - убран дубликат 'Морщины' (был и 'Морщины, возрастные изменения', и 'Морщины');
          // - объединены 'Неровный тон' + 'Пигментация' → 'Пигментация и неровный тон';
          // - 'Жирность и блеск' + 'Расширенные поры' → 'Жирность, блеск и расширенные поры';
          // - 'Отёки' + 'Круги под глазами' → 'Проблемы вокруг глаз';
          // - 'Чувствительность' → 'Чувствительность и покраснения' (объёмнее).
          // Скоринг в createScoreJson матчит по .includes('Акне' / 'Жирность' / 'Сухость' /
          ...
          options: [
            ...
            'В целом всё устраивает, нужен поддерживающий уход',
          ],
        },
        {
    ...
    // 17-20. Данные о здоровье
    {
      ...
      questions: [
        ...
        // Gate-вопрос: вместо того чтобы сразу показывать список из 13 ингредиентов
        // (большая часть юзеров не различает «AHA/BHA» от «парабенов»), сначала спрашиваем
        // короткое да/нет. Если «Нет» — пропускаем длинный multi_choice (см. filterQuestions.ts).
        {
          code: 'has_avoid_ingredients',
          text: 'Есть ли ингредиенты, которые вы хотите исключить из ухода?',
          type: 'single_choice',
          position: 4,
          isRequired: true,
          options: ['Да, есть', 'Нет, исключать ничего не нужно'],
        },
        {
          ...
          type: 'multi_choice',
          position: 5,
          isRequired: false, // Показывается только если has_avoid_ingredients === 'Да, есть'
          // Удалена опция «Такие ингредиенты отсутствуют» — gate-вопрос её заменил.
          options: [
            'Ретинол',
            'Витамин C',
